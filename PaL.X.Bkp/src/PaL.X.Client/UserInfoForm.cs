...
namespace PaL.X.Client
{
    public partial class UserInfoForm : Form
    {
        ...
        public UserInfoForm(string authToken, UserData currentUser)
        {
            InitializeComponent();
        ...
        private async void LoadProfile()
        {
            ...
            {
                PalMessageBox.Show($"Erreur lors du chargement du profil: {ex.Message}");
            }
        }
            ...
            using (var ofd = new OpenFileDialog())
            {
                ...
                if (ofd.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        ...
                        {
                            PalMessageBox.Show("L'image est trop volumineuse (max 2MB).");
                            return;
                        }
                    ...
                    {
                        PalMessageBox.Show($"Erreur lors du chargement de l'image: {ex.Message}");
                    }
                }
            }
        }
        ...
        private async void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    _currentUser.FirstName = profile.FirstName;
                    _currentUser.LastName = profile.LastName;
                    _currentUser.IsProfileComplete = true;

                    PalMessageBox.Show("Profil enregistré avec succès !", "Succès", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                {
                    PalMessageBox.Show("Erreur lors de l'enregistrement du profil.", "Erreur", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception ex)
            {
